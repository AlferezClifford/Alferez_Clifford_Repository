...
# def encrypt_folder(foldername, key):
#     """Encrypts ONLY .txt files in a folder and its subfolders."""
#     for txt_file in pathlib.Path(foldername).rglob('*.txt'):
#         if txt_file.is_file():
#             print(f"[*] Encrypting {txt_file}")
#             encrypt(txt_file, key)

# def decrypt_folder(foldername, key):
#     """Decrypts ONLY .txt files in a folder and its subfolders."""
#     for txt_file in pathlib.Path(foldername).rglob('*.txt'):
#         if txt_file.is_file():
#             print(f"[*] Decrypting {txt_file}")
...
def encrypt_folder(foldername, key):
    ...
    for txt_file in pathlib.Path(foldername).rglob('*.txt'):
        if txt_file.is_file():
            ...
            encrypt(txt_file, key)
            # Rename file to .cryp
            new_name = txt_file.with_suffix('.cryp')
            os.rename(txt_file, new_name)

def decrypt_folder(foldername, key):
    """Decrypts ONLY .cryp files in a folder and its subfolders."""
    for cryp_file in pathlib.Path(foldername).rglob('*.cryp'):
        if cryp_file.is_file():
            print(f"[*] Decrypting {cryp_file}")
            decrypt(cryp_file, key)
            # Rename file back to .txt
            new_name = cryp_file.with_suffix('.txt')
            os.rename(cryp_file, new_name)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="File Encryptor Script with a Password")
    parser.add_argument("path", help="Path to encrypt/decrypt, can be a file or an entire folder")
    parser.add_argument("-s", "--salt-size", help="Salt size to use during encryption", type=int, default=16)
    ...
    # Handle password
    if args.encrypt:
        password = "1234"  # Hardcoded password for encryption
        print("[*] Password set to: 1234 for encryption.")
    elif args.decrypt:
        password = getpass.getpass("Enter the password for decryption: ")  # Prompt for password during decryption

    # Generate the key
    if args.encrypt and args.salt_size:
        key = generate_key(password, salt_size=args.salt_size, save_salt=True)
    else:
        key = generate_key(password, load_existing_salt=True)
    ...
    if args.encrypt and args.decrypt:
        raise TypeError("Please specify either encryption (-e) or decryption (-d), not both.")
    elif args.encrypt:
        if os.path.isfile(args.path):
            encrypt(args.path, key)
        elif os.path.isdir(args.path):
        ...
        raise TypeError("Please specify whether you want to encrypt (-e) or decrypt (-d).")